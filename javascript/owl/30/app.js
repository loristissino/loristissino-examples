...
class Root extends Component {
    ...
    setup() {
        ...
        if (urlParams.has('url') && urlParams.has('id')) {
            this.remoteStore.url = urlParams.get('url');
            this.remoteStore.id = urlParams.get('id');
            this.load();
        }
    ...
    transfer() {
        ...
        let sourceIndex = this.accounts.findIndex(p => p.id == document.querySelector('#sourceAccountId').value.split(":")[0]);
        let targetIndex = this.accounts.findIndex(p => p.id == document.querySelector('#targetAccountId').value.split(":")[0]);
    ...
    async save() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.shareLinkRef.el.href = this.shareLink;
    ...
    get shareLink() {
        return window.location.origin + window.location.pathname + '?id=' + this.remoteStore.id + '&url=' + encodeURIComponent(this.remoteStore.url);
    }
