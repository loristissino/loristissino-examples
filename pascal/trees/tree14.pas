program tree14(Input,Output);

{
  * refactoring della procedura di ricerca del nodo padre, in modo da
  * non doverlo percorrere completamente...
  * }
   ...
   begin
    ...
    initSampleRecord(v[4], 3, 7, 'H1');
    initSampleRecord(v[5], 3, 8, 'P');
    initSampleRecord(v[6], 8, 9, 'EM');
    initSampleRecord(v[7], 9, 10, 'STRONG');
   end;
  ...
  begin
   ...
   begin
      if t<>nil then
        writeln('#debug: checking... ', t^.name);
      if (parent=nil) and (t<>nil) and (found=nil) then
        begin
